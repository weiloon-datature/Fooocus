...
def try_load_deprecated_user_path_config():
    ...
    try:
        ...
        if (
            deprecated_config_dict.get("default_model", None)
            == "juggernautXL_version6Rundiffusion.safetensors"
        ):
            os.replace("user_path_config.txt", "user_path_config-deprecated.txt")
            print(
                "Config updated successfully in silence. "
        ...
        if input(
            ...
            os.replace("user_path_config.txt", "user_path_config-deprecated.txt")
            print(
                "Config updated successfully by user. "
                'A backup of previous config is written to "user_path_config-deprecated.txt".'
            )
            return
    except Exception as e:
...
default_refiner_model_name = get_config_item_or_set_default(
    key="default_refiner", default_value="None", validator=lambda x: isinstance(x, str)
)
...
default_loras = get_config_item_or_set_default(
    ...
    and all(
        len(y) == 2 and isinstance(y[0], str) and isinstance(y[1], numbers.Number)
        for y in x
    ),
)
...
default_styles = get_config_item_or_set_default(
    ...
    validator=lambda x: isinstance(x, list)
    and all(y in modules.sdxl_styles.legal_style_names for y in x),
)
...
default_aspect_ratio = get_config_item_or_set_default(
    ...
    if "1152*896" in available_aspect_ratios
    else available_aspect_ratios[0],
...
config_dict["default_loras"] = default_loras = default_loras[:5] + [
    ["None", 1.0] for _ in range(5 - len(default_loras))
]
...
with open(config_example_path, "w", encoding="utf-8") as json_file:
    ...
    json_file.write(
        f'You can modify your "{cpa}" using the below keys, formats, and examples.\n'
...
def get_model_filenames(folder_path, name_filter=None):
    return get_files_from_folder(
        folder_path,
        [".pth", ".ckpt", ".bin", ".safetensors", ".fooocus.patch"],
        name_filter,
    )
...
def downloading_ip_adapters(v):
    ...
    if v == "face":
        ...
        results += [
            os.path.join(path_controlnet, "ip-adapter-plus-face_sdxl_vit-h.bin")
        ]
